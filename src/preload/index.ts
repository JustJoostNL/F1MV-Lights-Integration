...
export const f1mvli = {
  config: {
    set: (value: Partial<IConfig>): Promise<void> => ipcRenderer.invoke("f1mvli:config:set", value),
  ...
  updater: {
    checkForUpdates: (): Promise<void> => ipcRenderer.invoke("f1mvli:updater:checkForUpdates"),
    getUpdateAvailable: (): Promise<UpdateResult> => ipcRenderer.invoke("f1mvli:updater:getUpdateAvailable"),
    getForceDevUpdate: (): Promise<boolean> => ipcRenderer.invoke("f1mvli:updater:getForceDevUpdate"),
  },
  logger: {
  ...
  utils: {
    getIntegrationStates: () => ipcRenderer.invoke("f1mvli:utils:getIntegrationStates"),
    getWindowSizes: (): Promise<number[][]> => ipcRenderer.invoke("f1mvli:utils:getWindowSizes"),
    openNewWindow: (url: string): Promise<void> => ipcRenderer.invoke("f1mvli:utils:openNewWindow", url),
    changeWindowTitle: (title: string): Promise<void> => ipcRenderer.invoke("f1mvli:utils:changeWindowTitle", title),
    relaunchApp: (): Promise<void> => ipcRenderer.invoke("f1mvli:utils:relaunchApp"),
