...
export default function YeeLightDeviceAddFlow() {
  ...
  useEffect(() => {
    async function fetchDevices() {
      try {
        const data = await window.f1mvli.integrations.yeeLight.getDevices();
        setAlreadySelectedDevices(data);
      } catch (e) {
        log.error("Failed to fetch the YeeLight devices! Error: ", e);
      }
    }
    fetchDevices();
  }, []);
  ...
  useEffect(() => {
    ...
    window.f1mvli.config.set("Settings.yeeLightSettings.deviceIPs", deviceList);
  ...
  return (
    <div style={{ marginTop: "20px" }}>
      <TextField
        id="yeelight-device-ip-input"
        label="YeeLight Device IP"
        color="secondary"
        variant="outlined"
        value={ipInput}
