...
$(function() {
	ipcRenderer.on("homeAssistantAllInformation", (event, arg) => {
		const homeAssistantSelectedDevices = arg.homeAssistantSelectedDevices;
		const homeAssistantDeviceInformation = arg.deviceInformation;

		homeAssistantDeviceInformation.forEach((device) => {
			...
			document.getElementById("homeassistant-device-table").appendChild(newRow);
		});

		homeAssistantSelectedDevices.forEach((device) => {
			// get the device ID
			const deviceID = device;
			// get the table
			const table = document.getElementById("homeassistant-device-table");
			// get the rows
...
function saveSelectedDevices() {
	...
	ipcRenderer.send("homeAssistantSelectorSaveSelectedDevices", selectedDevices);
}